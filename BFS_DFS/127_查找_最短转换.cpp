...
//一次只能更改一个字母。
//每个转换的单词必须存在于单词列表中。需要注意的是beginWord是不是变换词。
...
int ladderLength(string beginword, string endword, vector<string>& worddict){
    ...
    while(!tovisit.empty()){
        ...
        for(int i=0;i<num;i++){
            ...
            tovisit.pop();
            if(word == endword) return dist;                //BFS 同一层级的深度是相同的 return dist也就是是最短的
            addNextWords(word,worddict,tovisit);            //构建word单词的下一个层级的queue
        }
        dist++;
    }
    return 0;
    ...
    for(int p=0;p<(int)word.length();p++){
        ...
        for(int k=0;k<26;k++){
            ...
            if(std::find(wordDict.begin(),wordDict.end(),word) != wordDict.end()){
                toVisit.push(word);
                wordDict.erase(std::remove(wordDict.begin(), wordDict.end(), word), wordDict.end());            //删除被使用过的word
            }
        }
        word[p] = letter;
    }
